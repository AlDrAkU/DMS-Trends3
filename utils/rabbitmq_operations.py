...
from utils.data_access.models import InvoiceItem, InvoiceSummary, InvoiceModel, EarningItem, DeductionItem, \
    PaycheckModel, \
    docTypeModel, FileModel
from utils.database.PostgresDatabase import PostgreSQLFileStorageRepository
from utils.util_funcs import build_response_message, map_to_json, get_config_directory
...
class RabbitMQOperations:
    ...
    def start_consumer():
        ...
        if connection:
            # Proceed with the rest of your code that uses the connection
            pass
        else:
            print("Connection not established. Cannot proceed.")
        #connection = pika.BlockingConnection(parameters)
